//! Pure request validation and response policy for account-balance.
...
pub fn rpc_request(account_address: &str, id: u8, commitment: &str) -> Value {
    ...
    json!({"jsonrpc":"2.0", "id":id, "method":"getBalance", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!("Solana RPC rejected getBalance: {error}"));
    }
    let result = response
        .get("result")
