//! Pure request validation and response policy for address-activity.
...
pub fn rpc_request(account_address: &str, id: u8, commitment: &str) -> Value {
    ...
    let _ = commitment;
    let params = json!([argument, {"commitment": commitment, "limit": 10}]);
    json!({"jsonrpc":"2.0", "id":id, "method":"getSignaturesForAddress", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!(
            "Solana RPC rejected getSignaturesForAddress: {error}"
        ));
    }
    let result = response
        .get("result")
