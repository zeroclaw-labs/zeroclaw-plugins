//! Pure request validation and response policy for block-commitment.
...
pub fn rpc_request(slot: u64, id: u8, commitment: &str) -> Value {
    ...
    let _ = commitment;
    let params = json!([argument.parse::<u64>().expect("slot is generated from u64")]);
    json!({"jsonrpc":"2.0", "id":id, "method":"getBlockCommitment", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!("Solana RPC rejected getBlockCommitment: {error}"));
    }
    let result = response
        .get("result")
...
pub fn build_report(query: String, result: Value) -> RpcReport {
    RpcReport {
        tool: "block-commitment",
        rpc_method: "getBlockCommitment",
        query,
        result,
    }
}
