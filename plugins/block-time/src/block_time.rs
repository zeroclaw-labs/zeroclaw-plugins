//! Pure request validation and response policy for block-time.
...
pub fn rpc_request(slot: u64, id: u8, commitment: &str) -> Value {
    ...
    json!({"jsonrpc":"2.0", "id":id, "method":"getBlockTime", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!("Solana RPC rejected getBlockTime: {error}"));
    }
    let result = response
        .get("result")
