//! Pure request validation and response policy for blockhash-validity.
...
pub fn rpc_request(blockhash: &str, id: u8, commitment: &str) -> Value {
    let argument = blockhash.to_string();
    let _ = commitment;
    let params = json!([argument, {"commitment": commitment}]);
    json!({"jsonrpc":"2.0", "id":id, "method":"isBlockhashValid", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!("Solana RPC rejected isBlockhashValid: {error}"));
    }
    let result = response
        .get("result")
...
pub fn build_report(query: String, result: Value) -> RpcReport {
    RpcReport {
        tool: "blockhash-validity",
        rpc_method: "isBlockhashValid",
        query,
        result,
    }
}
