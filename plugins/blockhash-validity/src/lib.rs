//! Check whether a public recent blockhash is still valid for transaction processing.

pub mod blockhash_validity;
...
mod component {
    ...
    const PLUGIN_NAME: &str = "blockhash-validity";
    ...
    impl Tool for Plugin {
        ...
        fn description() -> String {
            "Check whether a public recent blockhash is still valid for transaction processing. Read-only: cannot sign, fund, approve, or submit transactions.".to_string()
        }
        fn parameters_schema() -> String {
            serde_json::json!({
                "type":"object",
                "additionalProperties":false,
                "properties":{"blockhash":{"type":"string","minLength":32,"maxLength":88,"pattern":"^[1-9A-HJ-NP-Za-km-z]+$","description":"Base58-encoded public Solana identifier."}},
                "required":["blockhash"]
            }).to_string()
        }
        fn execute(args: String) -> Result<ToolResult, String> {
            ...
            if let Err(error) =
                validate_identifier(&parsed.blockhash, "blockhash", expected_decoded_len())
            {
            ...
            let request = rpc_request(&parsed.blockhash, 1, &config.commitment);
            ...
            let query = parsed.blockhash.to_string();
    ...
    fn emit(action: PluginAction, outcome: PluginOutcome, message: &str) {
        log_record(
            LogLevel::Info,
            &PluginEvent {
                function_name: "blockhash_validity::tool::execute".to_string(),
                action,
