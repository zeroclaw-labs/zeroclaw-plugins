...
use blockhash_validity::blockhash_validity::{
...
fn renders_a_bounded_json_report() {
    let report = build_report(
        "91cmfM46bCQ9fX8DQFCYzbhjjnKSeNAzADwDXZfpbazf".to_string(),
        json!({"value":42}),
    );
    let output = render_report(&report).unwrap();
    assert!(output.contains("blockhash-validity"));
    assert!(output.contains("isBlockhashValid"));
