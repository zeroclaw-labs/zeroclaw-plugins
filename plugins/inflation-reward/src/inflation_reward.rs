//! Pure request validation and response policy for inflation-reward.
...
pub fn rpc_request(account_address: &str, id: u8, commitment: &str) -> Value {
    let argument = account_address.to_string();
    let _ = commitment;
    let params = json!([[argument], {"commitment": commitment}]);
    json!({"jsonrpc":"2.0", "id":id, "method":"getInflationReward", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!("Solana RPC rejected getInflationReward: {error}"));
    }
    let result = response
        .get("result")
...
pub fn build_report(query: String, result: Value) -> RpcReport {
    RpcReport {
        tool: "inflation-reward",
        rpc_method: "getInflationReward",
        query,
        result,
    }
}
