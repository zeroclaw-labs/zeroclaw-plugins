//! Inspect confirmation, slot, and error state for a public Solana transaction signature.
...
mod component {
    ...
    const PLUGIN_NAME: &str = "signature-status";
    ...
    impl Tool for Plugin {
        ...
        fn description() -> String {
            "Inspect confirmation, slot, and error state for a public Solana transaction signature. Read-only: cannot sign, fund, approve, or submit transactions.".to_string()
        }
        fn parameters_schema() -> String {
            serde_json::json!({
                "type":"object",
                "additionalProperties":false,
                "properties":{"signature":{"type":"string","minLength":32,"maxLength":88,"pattern":"^[1-9A-HJ-NP-Za-km-z]+$","description":"Base58-encoded public Solana identifier."}},
                "required":["signature"]
            }).to_string()
        }
        fn execute(args: String) -> Result<ToolResult, String> {
            ...
            if let Err(error) =
                validate_identifier(&parsed.signature, "signature", expected_decoded_len())
            {
            ...
            let request = rpc_request(&parsed.signature, 1, &config.commitment);
            ...
            let query = parsed.signature.to_string();
    ...
    fn emit(action: PluginAction, outcome: PluginOutcome, message: &str) {
        log_record(
            LogLevel::Info,
            &PluginEvent {
                function_name: "signature_status::tool::execute".to_string(),
                action,
