//! Pure request validation and response policy for signature-status.
...
pub fn rpc_request(signature: &str, id: u8, commitment: &str) -> Value {
    let argument = signature.to_string();
    let _ = commitment;
    let params = json!([[argument], {"searchTransactionHistory": true}]);
    json!({"jsonrpc":"2.0", "id":id, "method":"getSignatureStatuses", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!("Solana RPC rejected getSignatureStatuses: {error}"));
    }
    let result = response
        .get("result")
