...
use serde_json::json;
use signature_status::signature_status::{
...
fn validates_endpoint_and_config() {
    ...
    assert!(validate_identifier(&base58_value(64), "signature", 64).is_ok());
    assert!(validate_identifier(&base58_value(32), "signature", 64).is_err());
}

#[test]
fn builds_the_bounded_read_only_request() {
    let value = base58_value(64);
...
fn renders_a_bounded_json_report() {
    let report = build_report(
        "3Bwv8nYQ7GbJvgCuRtqNzma3y5pCCqcRhENPHDTFuGbzgFyYodUp9echDd6iktfxQx8cpH3RpforaniwXEAaKxM6"
            .to_string(),
        json!({"value":42}),
    );
    let output = render_report(&report).unwrap();
    assert!(output.contains("signature-status"));
    assert!(output.contains("getSignatureStatuses"));
