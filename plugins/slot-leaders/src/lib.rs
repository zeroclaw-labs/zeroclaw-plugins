//! List the next ten scheduled leaders starting from a public Solana slot.

pub mod slot_leaders;
...
mod component {
    ...
    use crate::slot_leaders::{
        build_report, parse_rpc_response, render_report, rpc_request, RpcConfig,
    };
    ...
    const PLUGIN_NAME: &str = "slot-leaders";
    ...
    impl Tool for Plugin {
        ...
        fn description() -> String {
            "List the next ten scheduled leaders starting from a public Solana slot. Read-only: cannot sign, fund, approve, or submit transactions.".to_string()
        }
        fn parameters_schema() -> String {
            serde_json::json!({
                "type":"object",
                "additionalProperties":false,
                "properties":{"slot":{"type":"integer","minimum":0,"description":"Public Solana slot number."}},
                "required":["slot"]
            }).to_string()
        }
        fn execute(args: String) -> Result<ToolResult, String> {
            ...
            let request = rpc_request(parsed.slot, 1, &config.commitment);
            ...
            let query = parsed.slot.to_string();
    ...
    fn emit(action: PluginAction, outcome: PluginOutcome, message: &str) {
        log_record(
            LogLevel::Info,
            &PluginEvent {
                function_name: "slot_leaders::tool::execute".to_string(),
                action,
