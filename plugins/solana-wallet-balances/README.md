...
**A ZeroClaw WIT tool plugin** that tells your agent what a Solana address holds: SOL balance plus every non-zero SPL token balance, in one compact answer.

```
> what does JUP6…TaV4 hold?

SOL balance of JUP6…TaV4: 0.084 SOL (84000000 lamports)
JUP6…TaV4 — 18 SPL token(s) with non-zero balance:
  6D79fWsdioy3u7hH24aDDdUY294LBPQDgGTCMtFm8ZCd: 1,500
  423aBQ7TGKSJJP3yGRfh9efqAtUpVzzJaZQwGKf1Qkpn: 199
  … and 15 more (raised cap omitted to keep answers small)
```

*(real agent output — the tool made one `getBalance` + one `getTokenAccountsByOwner` call through the host's `wasi:http`)*

One tool, one job. Part of a three-plugin Solana read-only suite sharing [`solana-lens-core`](https://github.com/Acezhang08/solana-lens-core): **balances**, [activity](../solana-wallet-activity), [tx-inspect](../solana-tx-inspect).
