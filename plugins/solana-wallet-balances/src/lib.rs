//! A ZeroClaw WIT tool plugin: `solana_wallet_balances`.
//!
//! SOL + non-zero SPL token balances for any Solana address, shaped for the
//! model. Read-only (custody tier T0): the tool cannot sign, move funds, or
...
mod component {
    ...
    impl PluginInfo for Plugin {
        fn plugin_name() -> String {
            "solana-wallet-balances".to_string()
        }
    ...
    impl Tool for Plugin {
        ...
        fn execute(args: String) -> Result<ToolResult, String> {
            ...
            log_record(
                LogLevel::Info,
                &PluginEvent {
                    function_name: "solana_wallet_balances::tool::execute".into(),
                    ...
                    duration_ms: None,
                    attrs: None,
                    message: "wallet balances lookup".into(),
                },
            );
