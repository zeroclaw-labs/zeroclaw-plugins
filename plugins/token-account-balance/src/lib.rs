//! Inspect the amount and decimals held by a public SPL token account.
...
mod component {
    ...
    use crate::token_account_balance::{
    ...
    const PLUGIN_NAME: &str = "token-account-balance";
    ...
    impl Tool for Plugin {
        ...
        fn description() -> String {
            "Inspect the amount and decimals held by a public SPL token account. Read-only: cannot sign, fund, approve, or submit transactions.".to_string()
        }
        fn parameters_schema() -> String {
            serde_json::json!({
                "type":"object",
                "additionalProperties":false,
                "properties":{"token_account":{"type":"string","minLength":32,"maxLength":88,"pattern":"^[1-9A-HJ-NP-Za-km-z]+$","description":"Base58-encoded public Solana identifier."}},
        ...
        fn execute(args: String) -> Result<ToolResult, String> {
            ...
            let request = rpc_request(&parsed.token_account, 1, &config.commitment);
            ...
            let query = parsed.token_account.to_string();
    ...
    fn emit(action: PluginAction, outcome: PluginOutcome, message: &str) {
        log_record(
            LogLevel::Info,
            &PluginEvent {
                function_name: "token_account_balance::tool::execute".to_string(),
                action,
