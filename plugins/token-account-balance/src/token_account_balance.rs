//! Pure request validation and response policy for token-account-balance.
...
pub fn rpc_request(token_account: &str, id: u8, commitment: &str) -> Value {
    let argument = token_account.to_string();
    let _ = commitment;
    let params = json!([argument, {"commitment": commitment}]);
    json!({"jsonrpc":"2.0", "id":id, "method":"getTokenAccountBalance", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!(
            "Solana RPC rejected getTokenAccountBalance: {error}"
        ));
    }
    let result = response
        .get("result")
