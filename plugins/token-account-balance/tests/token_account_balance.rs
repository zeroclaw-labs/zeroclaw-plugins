...
use serde_json::json;
use token_account_balance::token_account_balance::{
...
fn renders_a_bounded_json_report() {
    let report = build_report(
        "AxTkKX5modwZPPs1FMhkKeuc9mtR9yQTQVzHDn5aag2u".to_string(),
        json!({"value":42}),
    );
    let output = render_report(&report).unwrap();
    assert!(output.contains("token-account-balance"));
    assert!(output.contains("getTokenAccountBalance"));
