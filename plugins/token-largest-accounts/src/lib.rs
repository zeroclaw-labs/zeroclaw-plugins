//! List the largest public token accounts for an SPL mint.

pub mod token_largest_accounts;
...
mod component {
    ...
    use crate::token_largest_accounts::{
    ...
    const PLUGIN_NAME: &str = "token-largest-accounts";
    ...
    impl Tool for Plugin {
        ...
        fn description() -> String {
            "List the largest public token accounts for an SPL mint. Read-only: cannot sign, fund, approve, or submit transactions.".to_string()
        }
        fn parameters_schema() -> String {
            serde_json::json!({
                "type":"object",
                "additionalProperties":false,
                "properties":{"mint_address":{"type":"string","minLength":32,"maxLength":88,"pattern":"^[1-9A-HJ-NP-Za-km-z]+$","description":"Base58-encoded public Solana identifier."}},
        ...
        fn execute(args: String) -> Result<ToolResult, String> {
            ...
            if let Err(error) =
                validate_identifier(&parsed.mint_address, "mint_address", expected_decoded_len())
            {
            ...
            let request = rpc_request(&parsed.mint_address, 1, &config.commitment);
            ...
            let query = parsed.mint_address.to_string();
    ...
    fn emit(action: PluginAction, outcome: PluginOutcome, message: &str) {
        log_record(
            LogLevel::Info,
            &PluginEvent {
                function_name: "token_largest_accounts::tool::execute".to_string(),
                action,
