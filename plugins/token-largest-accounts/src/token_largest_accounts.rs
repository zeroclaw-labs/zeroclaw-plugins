//! Pure request validation and response policy for token-largest-accounts.
...
pub fn rpc_request(mint_address: &str, id: u8, commitment: &str) -> Value {
    let argument = mint_address.to_string();
    let _ = commitment;
    let params = json!([argument, {"commitment": commitment}]);
    json!({"jsonrpc":"2.0", "id":id, "method":"getTokenLargestAccounts", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!(
            "Solana RPC rejected getTokenLargestAccounts: {error}"
        ));
    }
    let result = response
        .get("result")
