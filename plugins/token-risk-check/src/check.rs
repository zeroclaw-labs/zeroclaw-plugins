...
/// Make an `https://` RPC URL survive the plugin HTTP sandbox by pinning the
/// default port explicitly.
///
/// Inside a tool plugin, `waki` → `wasi:http` → `default-send-request` does
/// not restore the scheme's default port: an `https://` URL with no explicit
/// port dials 80 and is refused before TLS, surfacing as a generic
/// `ConnectionRefused` that looks like the endpoint being down. So we insert
/// `:443` when the authority has no port of its own. A URL that already names
/// a port is left untouched.
pub fn normalize_rpc_url(url: &str) -> String {
    ...
    // A ':' in the authority means a port is already set — but skip the
    // IPv6-literal case (`[::1]`), which contains colons inside brackets.
    let has_port = if let Some(close) = authority.rfind(']') {
        authority[close + 1..].contains(':')
    } else {
        authority.contains(':')
    };
    if authority.is_empty() || has_port {
        ...
        format!("{SCHEME}{authority}:443{tail}")
    }
}
...
pub fn run_check(
    ...
    // When the largest accounts ARE available, resolve them to wallet owners
    // and aggregate per owner; if that second call fails, fall back to raw
    // per-token-account math and let the report say which basis was used.
    let conc = rpc
        .get_token_largest_accounts(&mint_addr, commitment)
        .ok()
        .and_then(|accounts| {
            if accounts.is_empty() {
                return holders::concentration(&accounts, facts.supply);
            }
            let addresses: Vec<String> = accounts.iter().map(|a| a.address.clone()).collect();
            match rpc.get_token_account_owners(&addresses, commitment) {
                Ok(owners) => holders::concentration_by_owner(&accounts, &owners, facts.supply),
                Err(_) => holders::concentration(&accounts, facts.supply),
            }
        });
