...
mod component {
    ...
    impl Transport for WakiTransport {
        fn send(&self, body: &serde_json::Value) -> Result<serde_json::Value, String> {
            /// A hostile or broken RPC must not be able to balloon the call:
            /// anything past this is discarded unparsed. (The read itself is
            /// additionally bounded by the host's per-call memory ceiling.)
            ...
            let bytes = resp.body().map_err(|e| format!("RPC read failed: {e}"))?;
            if bytes.len() > MAX_RESPONSE_BYTES {
                return Err(format!(
                    "RPC response exceeds the {} KiB cap",
                    MAX_RESPONSE_BYTES / 1024
                ));
            }
            serde_json::from_slice(&bytes).map_err(|e| format!("RPC returned non-JSON: {e}"))
        }
    }
