...
pub fn render(input: &ReportInput) -> String {
    ...
    if let Some(c) = concentration {
        let basis = if c.owner_resolved {
            "owner-resolved; pools/exchanges may still aggregate many users"
        } else {
            "per token account; one wallet may own several"
        };
        snapshot.push_str(&format!(
            "; top1 {:.1}%, top5 {:.1}%, top10 {:.1}% of supply ({basis})",
            c.top1_pct, c.top5_pct, c.top10_pct
        ));
        if let Some(p) = c.pool_share_pct {
            snapshot.push_str(&format!(
                "; known DEX pool vaults hold {p:.1}% (liquidity, excluded from whale metrics)"
            ));
        }
    }
    snapshot.push('.');
