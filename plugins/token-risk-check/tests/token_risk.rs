...
#[test]
fn parses_bonk_revoked_mint_authority_with_stale_payload() {
    let bytes = [
        0x00, 0x00, 0x00, 0x00, 0x79, 0x59, 0x51, 0x67, 0xda, 0x48, 0x0c, 0x5a, 0xe1, 0x34, 0x45,
        0x01, 0xd2, 0x11, 0xb7, 0x73, 0x63, 0x40, 0xe3, 0xfb, 0xdf, 0x00, 0xec, 0xde, 0x63, 0xb6,
        0x4d, 0xc8, 0x8a, 0xcc, 0x2f, 0x1c, 0xdb, 0x26, 0x87, 0xf3, 0x74, 0xf6, 0x1d, 0x7a, 0x05,
        0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ];
    let mint = parse_mint_account(TOKEN_PROGRAM_ID, &bytes).unwrap();
    assert!(mint.mint_authority.is_none());
    assert!(mint.freeze_authority.is_none());
}

#[test]
fn parses_revoked_freeze_authority_with_stale_payload() {
    let mut bytes = legacy_mint(None, None);
    bytes[50] = 1;
    let mint = parse_mint_account(TOKEN_PROGRAM_ID, &bytes).unwrap();
    assert!(mint.freeze_authority.is_none());
