//! Pure request validation and response policy for token-supply.
...
pub fn rpc_request(mint_address: &str, id: u8, commitment: &str) -> Value {
    ...
    json!({"jsonrpc":"2.0", "id":id, "method":"getTokenSupply", "params":params})
}

pub fn parse_rpc_response(response: &Value) -> Result<Value, String> {
    if let Some(error) = response.get("error") {
        return Err(format!("Solana RPC rejected getTokenSupply: {error}"));
    }
    let result = response
        .get("result")
